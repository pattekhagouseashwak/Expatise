...
const uploadBidderProfile = async (req, res) => {
  try {
    ...
    await Bidder.findOneAndUpdate({_id:id},
              ...
              .select("-password -is_EmailVerified -otp -Expiry_time -is_PhoneVerified -createdAt -updatedAt -Email_Expiry_time -Email_otp")
              .then((data)=>{
                            res.status(200).send({ status: 200, message: "successfully Bidder profile Photo has updated!!",data})
                            }
                                        ).catch(Err => {
                                            res.status(500).send({
                                            status: 500,
                                            message:
                                                Err.message || "Some error occurred while updating Bidder Profile Photo."
                                            });
                                        });
  } catch (error) {
    console.log(error)
...
module.exports = { uploadAuctioneerProfile,uploadBidderProfile }
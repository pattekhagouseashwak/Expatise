...
const webDevServices = async (req, res) => {
        try {      
          ...
          await WebDevServices.create({InqueryNo,InqurierType,InqurierName,phoneNumber,customer,email,newLetter_status})
                          .then(async (data) => {
                                                 let host = req.get('host');
                                                 console.log("host:", host);
                                                 await sendEmailToCustomer(host, data.email, "NA",3, emailConstants.RequestForWebsiteDevelopment, emailConstants.htmlcontent_WEBSITEDEVELOPMENTSERVICES, data.InqurierName,emailConfig.username_bussiness);

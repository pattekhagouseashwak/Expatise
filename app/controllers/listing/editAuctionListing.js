...
exports.fetchAuctionListing = async (obj_id) => {
    ...
    var cursor;
    await AuctionListing.findById({ _id: obj_id }).then(data => {
      cursor = data;
    })
      .catch(err => {
        cursor = null;
...
const editAuctionListing = async (req, res) => {//console.log(req.body.id )
  try {
    ...
    //console.log(resultSet.Auctioneer)

    let data =await this.fetchAuctioneer(resultSet.Auctioneer);

    //console.log(data)

    const Auctioneer = resultSet.Auctioneer;
